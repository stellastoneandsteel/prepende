...
Default provider is `echo` so Prepende runs out of the box with no key and no
...
def mirror_brand_env() -> None:
    """Mirror deprecated aliases while making Prepende values authoritative."""

    mirror_brand_environment()
...
def load_dotenv(path: str = ".env") -> None:
    ...
    if p.exists():
        for raw in p.read_text().splitlines():
            ...
            # keys in Prepende's shell (SEPARATION.md); .env is the intended source.
...
class Config:
    def __init__(self) -> None:
        ...
        self.model_route = (
            brand_env("MODEL_ROUTE")
            or "anthropic,openai,grok,google"
        ).strip()
        self.anthropic_model = (
            brand_env("ANTHROPIC_MODEL")
            or ""
        ).strip()
        self.openai_model = (
            brand_env("OPENAI_MODEL")
            or ""
        ).strip()
        self.grok_model = (
            brand_env("GROK_MODEL")
            or "grok-2-latest"
        ).strip()
        self.google_model = (
            brand_env("GOOGLE_MODEL")
            or ""
        ).strip()
        ...
        self.embedding_model = (g("EMBEDDING_MODEL", "") or brand_env("EMBEDDING_MODEL") or "").strip()
        ...
        self.embedding_dim = int((g("EMBEDDING_DIM", "") or brand_env("EMBEDDING_DIMENSIONS") or "1536") or 1536)
        ...
        self.vault_master_key = brand_env("KEY_VAULT_MASTER_KEY")
